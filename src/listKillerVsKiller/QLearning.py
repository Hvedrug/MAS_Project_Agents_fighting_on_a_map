import numpy as np
import src.listKillerVsKiller.Policy as Policy
...
def test(env, q_table, num_episodes=10):
	...
	for _ in range(num_episodes):
		...
		while not done:
			...
			actions[1] = Policy.bestFirstPolicy(q_table, state, 1)
			
			if actions == env.lastActions:
				#if looping on the same action then random moves for both
				if np.random.uniform(0, 1)<0.5:
					actions[0] = Policy.randomPolicy(env)
				else:
					actions[1] = Policy.randomPolicy(env)
		...
		while not done:
			if np.random.uniform(0, 1)<epsilon:
				if np.random.uniform(0, 1)<0.5:
					...
					actions[1] = Policy.bestFirstPolicy(q_table, state, 0)
				else:
					actions[1] = Policy.randomPolicy(env)
					#if less than epsilon, pick a random action
					actions[0] = Policy.bestFirstPolicy(q_table, state, 0)
			...
			#update Q_table with what we get for agent1
			old_value = q_table[state, actions[0], 0]
			next_max = np.max(q_table[next_state])
			q_table[state, actions[0], 0] = (1-alpha)*old_value + alpha*(rewards[0]+gamma*next_max)
			#same with agent2
			old_value = q_table[state, actions[1], 1]
			next_max = np.max(q_table[next_state])
			q_table[state, actions[1], 1] = (1-alpha)*old_value + alpha*(rewards[1]+gamma*next_max)
